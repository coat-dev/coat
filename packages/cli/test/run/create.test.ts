...
describe("coat run - create", () => {
  test("should work as a local dependency in a freshly created coat project", async () => {
    const { cwd, task: createTask } = runCli([
      "create",
      "coat-dev/cli-integration-tests-template#coat/integration-test-template-run-1-v1.0.0",
      "project-name",
    ]);
    await createTask;
    ...
    if (!process.env.COAT_CLI_TMP_TARBALL_PATH) {
      throw new Error(
        "Environment variable COAT_CLI_TMP_TARBALL_PATH must be set by integration test setup in order for this test to run as expected."
      );
    }
