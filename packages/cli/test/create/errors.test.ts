...
describe("coat create - errors", () => {
  test.concurrent(
    "should throw error if no template is specified",
    async () => {
      try {
  ...
  test.concurrent(
    "should throw error if template does not exist in registry",
    async () => {
      try {
        const { task } = runCli([
          "create",
  ...
  test.concurrent(
    "should not create target directory and files if template does not exist in registry",
    async () => {
  ...
  test.concurrent(
    "should throw error if target directory is not empty",
    async () => {
      ...
      try {
        const { task } = runCli(
          ["create", "@coat/template-ts-package", "project-name"],
          tmpDir
        );
        await task;
